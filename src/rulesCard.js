...
function rulesManagerCard(e, action, search, days) {
  ...
  if (typeof rules === "string") {
    var selectRulesBodyWidget = CardService.newTextParagraph().setText(rules);
  } else {
    var selectRulesBodyWidget = CardService.newSelectionInput()
      .setType(CardService.SelectionInputType.DROPDOWN)
      .setFieldName("editRule")
      .setOnChangeAction(
        CardService.newAction().setFunctionName("onModeChange")
      )
  ...
  const selectRulesBodyText = CardService.newTextParagraph().setText(
    "<b>Which rule do you want to edit?</b>"
  );
  ...
  if (
    ...
    const _searchText = CardService.newTextParagraph().setText(
      '<b>Use <a href="https://support.google.com/mail/answer/7190?hl=en">GMail™ Search String</a> to tell MailMaid which messages need to be removed:</b>'
    );
    ...
    const _daysText = CardService.newTextParagraph().setText(
      "<b>How many days before MailMaid cleans the messages?</b>"
    );
    ...
    const _actionText = CardService.newTextParagraph().setText(
      "<b>What do you want MailMaid to do?</b>"
    );
    ...
    var editRuleNumWidget =
      CardService.newTextParagraph().setText(editRuleNumText);
    ...
    const _daysText = CardService.newTextParagraph().setText(
      "<b>How many days until before MailMaid cleans the messages?</b>"
    );
    ...
    const _actionHint = CardService.newTextParagraph().setText(
      '<font color="#bcbcbc">Purge moves to Trash \nArchive removes from Inbox</font>'
    );
...
function onModeChange(e) {
  try {
    ...
    Logger.log(
      `${user} - Returning onModeChange array of:${action}, ${search}, ${days}`
    );
    return rulesManagerCard(e, action, search, days);
  } catch (e) {
    Logger.log(
      `${user} - onModeChange failed on processing ${ruleNum}: ${e.message}`
    );
  }
}
