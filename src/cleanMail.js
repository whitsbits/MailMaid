...
function cleanMail() {
  ...
  rulesloop: for (let i = rulesCached; i < rules.length; i++) {
    if (!checkAuth()) {
      loopBreak = 1;
      break rulesloop;
    }
    ...
    try {
      searchloop: while (loopBreak === 0) {
        ...
        for (let j = threadsCount; j < threads.length; j++) {
          //Start looping the messages in threads
          ...
          if (isTimeUp_(scriptStart, timeOutLimit)) {
            ...
            Logger.log(
              `${user} - Timed out in Rule ${i}, Batch start ${searchBatchStart} and Thread ${j} with partial count of ${counter}. Values put in cache`
            );
            ...
            if (triggerActive("cleanMore") === false) {
              Logger.log(
                `${user} - Setting a trigger to call the cleanMore function.`
              );
      ...
      if (e.message.includes("Mail service not enabled")) {
        //kill deadhead for non addon enabled accounts
        Logger.log(`${user} - Removing triggers for dead head account`);
        removeTriggers("MailMaid");
        removeTriggers("checkTrigger");
      } else if (
        e.message.includes("Service invoked too many times for one day")
      ) {
