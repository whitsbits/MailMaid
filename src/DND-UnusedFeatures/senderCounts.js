...
function countSenders(
  ...
  searchloop: try {
    do {
      ...
      Logger.log(
        `${user} - Processing batch ${batch} starting at thread ${threadsCount}.`
      );
      if (threads.length === 0) {
        break searchloop;
      }
      ...
      for (var i = threadsCount; i < threads.length; i++) {
        ...
        if (isTimeUp_(scriptStart, timeOutLimit)) {
          ...
          Logger.log(
            `${user} - Timed out in Thread ${i}, Batch start ${searchBatchStart}. Values put in cache`
          );
  ...
  if (loopBreak !== 1) {
    sender_array.forEach(function (r) {
      // add the message counts to each sender in the the sender_array
    ...
    for (var i = 0; i < topValues.length; i++) {
      // stamp out the actual search string the user will paste into the UI
