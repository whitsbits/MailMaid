...
const whiteSpace = CardService.newTextParagraph().setText("\n");
...
function actionSection() {
  const actionBodyText =
    CardService.newTextParagraph().setText("<b>Actions</b>");
...
function homepageRulesSection() {
  ...
  const rulesBodyText = CardService.newTextParagraph().setText(rulesText);
...
function homepageLicenseSection() {
  ...
  const licenseBodyText =
    CardService.newTextParagraph().setText("<b>License</b>");
  ...
  const upgradeLicenseButton = CardService.newTextButton()
    ...
    .setOpenLink(
      CardService.newOpenLink()
        .setUrl(
          "https://us-central1-mailamidweb.cloudfunctions.net/getSessionUrl"
        )
...
function disclosuresSection() {
  ...
  const disclosureTextParagraph =
    CardService.newTextParagraph().setText(disclosureText);

  const disclosuresSection = CardService.newCardSection().addWidget(
    disclosureTextParagraph
  );
...
function gotoPreviousCard() {
  var nav = CardService.newNavigation().popCard().updateCard(onHomepage());
...
function gotoRootCard() {
  var nav = CardService.newNavigation().popToRoot().updateCard(onHomepage());
...
function navFooter() {
  ...
  var toRootButton = CardService.newTextButton()
    .setText("HOME")
    .setTextButtonStyle(CardService.TextButtonStyle.FILLED)
    .setOnClickAction(CardService.newAction().setFunctionName("gotoRootCard"));

  // Return a new fixedFooter containing these two buttons.
  var fixedFooter = CardService.newFixedFooter().setPrimaryButton(toRootButton);
