...
function downloadToDrive(
  ...
  Logger.log(
    `${user} - Processing ${downloadAction} with ${search} as type ${fileTypeAction} to ${saveFile} with atchDownload=${atchDownload} and parseReply=${parseReply}.`
  );
  var threads = GmailApp.search(search, 0, inc);
  var slug = printSlug();
  var foldeName =
    "MailMaid Download" + "_" + search + "_" + epochDateConverter(Date.now());
    ...
    for (var j = 0; j < messages.length; j++) {
      ...
      if (atchDownload) {
        ...
        for (var k in attachments) {
          ...
          var atchSuffix = atchName.substring(
            atchName.indexOf("."),
            atchName.length
          );
...
Cc: ${msg.getCc()} \n\n`;
  ...
  sendReportEmail(
    "MailMaid Download Complete",
    "src/download-email.html",
    false,
    licenseRead(),
    messages.length,
    [
      `The files from the search of: ${search}`,
      `Have been downloaded into Drive folder: ${folderlink}`,
    ]
  );
}
